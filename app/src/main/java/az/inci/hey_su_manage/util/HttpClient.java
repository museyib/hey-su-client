...
import static az.inci.hey_su_manage.util.GlobalParameters.jwt;
...
import az.inci.hey_su_manage.model.CustomResponse;
import az.inci.hey_su_manage.model.ResponseMessage;
import az.inci.hey_su_manage.security.AuthenticationRequest;
import az.inci.hey_su_manage.security.AuthenticationResponse;
...
public class HttpClient {
    ...
    public String getNewToken() {
        ...
        try(Response response = sendRequest(new URL(serviceURL + "/authenticate"), "POST", request)) {
            ResponseBody responseBody = response.body();
            Gson gson = new Gson();
            AuthenticationResponse authenticationResponse =
                    gson.fromJson(responseBody.string(), new TypeToken<AuthenticationResponse>() {}.getType());
    ...
    private Response sendRequest(URL url, String method, @Nullable Object requestBodyData) throws IOException {
        ...
        if (method.equals("POST")) {
            RequestBody requestBody = RequestBody.create(new Gson().toJson(requestBodyData), MediaType.get("application/json;charset=UTF-8"));
        ...
        if (response.code() == 403) {
            ...
            response = sendRequest(url, method, requestBodyData);
        }
        return response;
    }
